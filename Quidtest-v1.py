...
#TODO edit code so that the user can select by match_name then select by game
#TODO create 3 save files one for players one for teams one for match_name
...
if os.path.exists('./games/team/')==False:
    os.mkdir('./games/team/')
if os.path.exists('./games/match_name/')==False:
    os.mkdir('./games/match_name/')
...
def select_match():
    ...
    global df_match
    global playerlist
    df_match=pd.read_csv('./games/game_def/'+match.get())
...
opt_select_player.config(width=20,relief=tk.GROOVE)
...
opt_select_player['state']='disabled'
...
def window_add_match():
        ...
        if not ent_match_name.get()=='' and not ent_team1.get()=='' and not ent_team2.get()=='' and not lstbx_team1.size()==0 and not lstbx_team2.size()==0:
            ...
            for n in range(lstbx_team2.size()): #iterates across all items in the listbox
                ...
                if item[0].strip() in df_match.Name.values:
                    ...
                    df_match=df_match.append({'Name':(item[0]),
                                              'Team':ent_team2.get(),'Position':item[1]},ignore_index=True)
            if os.path.exists('./games/game_def/'+ent_match_name.get()+ent_year.get())==False:
                os.mkdir('./games/game_def/'+ent_match_name.get()+ent_year.get())
    ...
    lbl_match_name=tk.Label(wd_add_match,text='match_name')
