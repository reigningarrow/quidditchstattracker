...
def add_attempt(method):
    ...
    try:
        ...
        if (method=='catch' or method=='block' or method=='drop') and player.get!=prev_name:
            
            dataframe=dataframe.append({'name':prev_name,'team':team.get(),'event':'pass end','coords':coords.get()},ignore_index=True)
    except:
        pass
    ...
    for kind in ['team','name']:
        ...
        for team in dataframe[kind].unique():
            ...
            graphs=[['shot','goal'],['pass','catch','pass end','drop','block'],
                    ['catch'],['block'],['drop'],['beater'],['sk catch']]
            ...
            for item in graphs:
                ...
                for graph in ['heat','circle']:
                    ...
                    for i,data in df.iterrows():
                        ...
                        '''
                        #plots 2 types of graph a circle and a heatmap
                        #heatmap may be discarded in future as not needed
                        if graph=='circle':
                            ...
                            if d2['event']=='shot':
                                #finds the position the event happend in the 2d array and adds 1
                        ...
                        #print('x',x)
                        #print('y',y,'\n')
                        elif item==['pass','catch','pass end','drop','block']:
                            if data['event']=='pass':
                                continue
                            else:
                                pass_start_loc=d2['coords'].iloc[i-1].split()
                                y=int(data[0])*33/15
                                x=int(data[-1])*60/30-0.5
               ...
               arrowprops=dict(arrowstyle="->",connectionstyle="arc3", color = 'red'))
                    ...
                    if graph=='heat':
                        if item==['shot','goal']:
                            #choose different colours based on type of event
